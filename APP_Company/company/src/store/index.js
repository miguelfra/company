...
export default new Vuex.Store({
  ...
  actions: {
    ...
    async GETProductsAction(context){
      const products = await axios('https://company-api-v2.herokuapp.com/api/products', {
        headers: {
    ...
    async newProduct(context, product) {
       axios.post('https://company-api-v2.herokuapp.com/api/products', product, {
        headers: {
          "x-access-token":  token
        }
       })
    ...
    async EditProduct(context, product) {
      axios.put(`https://company-api-v2.herokuapp.com/api/products/${product.id}`, product, {
       headers: {
         "x-access-token":  token
       }
      })
